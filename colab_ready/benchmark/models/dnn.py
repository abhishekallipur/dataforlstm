"""
Deep Neural Network (DNN) forecaster with batch normalization and dropout.
"""
...
from benchmark.config import DNNConfig, set_global_seed
from benchmark.models.base import BaseForecaster

logger = logging.getLogger("benchmark.models.dnn")


class DNNForecaster(BaseForecaster):
    """Deep fully-connected network with BatchNorm + Dropout."""

    name = "DNN"
    model_type = "tabular"

    def __init__(self, config: Optional[DNNConfig] = None) -> None:
        self.cfg = config or DNNConfig()
        self.model = None
    ...
    def _build(self, n_features: int):
        ...
        x = inputs
        for units, drop in zip(self.cfg.hidden_units, self.cfg.dropout_rates):
        ...
        model = Model(inputs, outputs, name="dnn_ghi")
        model.compile(
    ...
    def fit(
        ...
        **kwargs: Any,
        ...
        logger.info(
            "Training %s — layers=%s, bn=%s, epochs=%d",
            self.name, self.cfg.hidden_units, self.cfg.use_batch_norm, self.cfg.epochs,
        )
    ...
    def get_params(self) -> Dict[str, Any]:
        return {
            ...
            "dropout_rates": self.cfg.dropout_rates,
    ...
    def save(self, directory: str) -> None:
        ...
        if self.model is not None:
            path = os.path.join(directory, "dnn_model.h5")
            self.model.save(path)
            logger.info("Saved DNN model to %s", path)
